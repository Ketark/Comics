...
export default function ComicPage() {
  const { id } = useParams();
  const { data, isError } = comicAPI.useFetchOneComicsQuery(id);
  ...
  return (
      <article className={`comicContainer ${Loaded ? "activeCom" : ""}`} onLoad={() => setLoaded(true)}>
        <article className="oneComic" key={comic?.id}>
          ...
          <div className="info">
            ...
            <div>
              Description:{" "}
              {comic?.textObjects[0]?.text.replace(/<br>/gi, "") ||
                "не указано"}
            </div>
          </div>
